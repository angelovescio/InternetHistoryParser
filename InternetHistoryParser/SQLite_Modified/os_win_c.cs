...
*/
        static int seekWinFile(sqlite3_file id, sqlite3_int64 iOffset)
        {
            ...
            */
            ...
            try
            {
                if (!id.zPath.EndsWith("-journal") || !id.zPath.EndsWith("sqlite"))
                {
                    ms.Seek(iOffset, SeekOrigin.Begin);
                }
                else
                {
...
free(pFile.zDeleteOnClose);
      ...
      if ( SimulateIOError() )
        return SQLITE_IOERR_READ;
            ...
            try
            {
                if (!id.zPath.EndsWith("-journal") && !id.zPath.EndsWith("sqlite") && id.zPath != "temp")
                {
                    nRead = ms.Read(pBuf, 0, amt); // i  if( null==ReadFile(pFile->h, pBuf, amt, &nRead, 0) ){
                }
                else
                {
