    ...
    */
    ...
    )
    {
      ...
      FileMode dwCreationDisposition;
        SafeFileHandle shFile = new SafeFileHandle(IntPtr.Zero,true);
#if !(SQLITE_SILVERLIGHT || WINDOWS_MOBILE)
      FileOptions dwFlagsAndAttributes;
      WinapiMarshal.Functions funcs = new WinapiMarshal.Functions();
#endif
#if SQLITE_OS_WINCE
int isTemp = 0;
#endif
...
dwFlagsAndAttributes |= FileOptions.RandomAccess; // FILE_FLAG_RANDOM_ACCESS;
      ...
      if ( isNT() )
      {
        ...
        //
        int retries = 3;
        hFile = funcs.pFuncCreateFile(zName, (FileAccess)dwDesiredAccess, (FileShare)dwShareMode, IntPtr.Zero, dwCreationDisposition, 0, IntPtr.Zero);
        shFile = new SafeFileHandle(hFile, true);
        while (shFile.IsInvalid && (retries > 0))
          try
          {
            retries--;
#if WINDOWS_PHONE || SQLITE_SILVERLIGHT  
 fs = new IsolatedStorageFileStream(zConverted, dwCreationDisposition, dwDesiredAccess, dwShareMode, IsolatedStorageFile.GetUserStoreForApplication());
      ...
      shFile.IsInvalid == true ? "failed" : "ok" );
      if ( shFile.IsInvalid == true
      //||
#if !(SQLITE_SILVERLIGHT || WINDOWS_MOBILE)
...
pFile.lastErrno = 1;
      ...
      pFile.fs = new FileStream(shFile,FileAccess.Read);
